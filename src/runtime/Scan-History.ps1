...
$home=Get-CodexHome;$factory=Get-FactoryHome;$dict=New-Object 'Collections.Generic.Dictionary[string,string]' ([StringComparer]::OrdinalIgnoreCase);$files=@()
try{$files=Get-ChildItem -LiteralPath $home -Recurse -Filter *.jsonl -File -ErrorAction SilentlyContinue|Where-Object{$_.FullName -notlike "$factory*"}|Sort-Object LastWriteTime -Descending}catch{}
