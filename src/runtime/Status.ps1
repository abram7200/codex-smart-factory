...
$codexHome=Get-CodexHome;$factory=Get-FactoryHome;$override=Join-Path $codexHome "AGENTS.override.md";$coreText=Read-Utf8 $override;$coreOn=$coreText -match 'CSF_CORE_ID=codex-smart-factory-final-1\.1\.0'
...
try{$files=Get-ChildItem -LiteralPath $codexHome -Recurse -Filter *.jsonl -File -ErrorAction SilentlyContinue|Where-Object{$_.FullName -notlike "$factory*"}|Sort-Object LastWriteTime -Descending|Select-Object -First 30}catch{}
...
Write-Output ("CODEX_HOME: "+$codexHome);if($root){Write-Output ("Project: "+$root)};if($matchingRollout){Write-Output ("Evidence rollout: "+$matchingRollout)}
