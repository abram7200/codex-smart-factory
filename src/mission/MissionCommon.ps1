...
function Read-MissionState([string]$Root){
  ...
  try{return (Get-Content -Raw -LiteralPath $path|ConvertFrom-Json)}catch{throw ("Mission state is unreadable: {0}" -f $path)}
}
...
function Append-Progress([string]$Root,[string]$Text){
  ...
  if(!(Test-Path -LiteralPath $path)){Write-Utf8 $path "# Progress`r`n`r`n"}
...
function Append-Finding([string]$Root,[string]$Text){
  ...
  if(!(Test-Path -LiteralPath $path)){Write-Utf8 $path "# Findings and decisions`r`n`r`n"}
  $stamp=(Get-Date).ToString("yyyy-MM-dd HH:mm:ss K")
  $entry="## {0}`r`n`r`n{1}`r`n" -f $stamp,$Text.Trim()
  Add-Content -Encoding UTF8 -LiteralPath $path -Value $entry
}
...
function Get-QuotaDecision($State,[string]$RouteClass="balanced",[string]$Effort="medium",[switch]$Fresh){
  ...
  return [pscustomobject]@{action=$action;reason=$reason;quota=$q;session_minutes=[Math]::Round($minutes,1)}
}
...
function Indent-Block([object[]]$Value){
  ...
  $indented=New-Object 'System.Collections.Generic.List[string]'
  foreach($item in $items){$indented.Add(("    "+([string]$item)))}
...
function Render-TaskCard([string]$Root,$Task){
  ...
  $lines.Add(("# {0} - {1}" -f $Task.id,$Task.title));$lines.Add("")
  ...
  $lines.Add(("- Risk: {0}" -f $Task.risk));$lines.Add("")
  ...
  $lines.Add(("- Model actually used: {0}" -f $Task.model_used));$lines.Add("")
  ...
  $lines.Add(("- Created: {0}" -f $Task.created_at));$lines.Add(("- Updated: {0}" -f $Task.updated_at))
...
function Render-MissionDocs([string]$Root,$State,[string]$CheckpointReason="state-update"){
  ...
  Update-ReadyTasks $State
  $q=$State.quota
  $five="unknown";$week="unknown"
  if($q -and $q.five_hour){$five=("{0:N1}%" -f [double]$q.five_hour.remaining_percent)}
  if($q -and $q.weekly){$week=("{0:N1}%" -f [double]$q.weekly.remaining_percent)}

  $mission=New-Object 'System.Collections.Generic.List[string]'
  $mission.Add("# Codex Smart Factory Mission");$mission.Add("")
  $mission.Add(("- Mission ID: {0}" -f $State.mission_id));$mission.Add(("- Status: **{0}**" -f $State.status))
  $mission.Add(("- Current gate: {0}" -f $State.current_gate));$mission.Add(("- Execution mode: {0}" -f $State.mode))
  $mission.Add(("- Active task: {0}" -f $State.active_task_id));$mission.Add(("- Safe stop requested: {0}" -f $State.safe_stop_requested))
  $mission.Add(("- Safe stop reason: {0}" -f $State.safe_stop_reason));$mission.Add(("- 5h remaining: **{0}**" -f $five))
  $mission.Add(("- Weekly remaining: **{0}**" -f $week));$mission.Add(("- Last checkpoint: {0}" -f $State.last_checkpoint_at));$mission.Add("")
  ...
  $table.Add("# Task graph and checklist");$table.Add("")
  ...
  foreach($task in @($State.tasks)){
    $cells=@((Markdown-Safe $task.id),(Markdown-Safe $task.status),(Markdown-Safe $task.title),(Markdown-Safe $task.why),(Markdown-Safe (Join-List $task.depends_on)),(Markdown-Safe (Join-List $task.related)),(Markdown-Safe $task.relation),(Markdown-Safe $task.discovered_from),(Markdown-Safe $task.best_model),(Markdown-Safe $task.effort),(Markdown-Safe $task.model_reason),(Markdown-Safe $task.model_used),(Markdown-Safe $task.scope),(Markdown-Safe $task.verify),(Markdown-Safe $task.updated_at))
    $table.Add("| "+($cells -join " | ")+" |")
    Render-TaskCard $Root $task
  }
  Write-Utf8 (Join-Path $dir "TASKS.md") (($table -join "`r`n")+"`r`n")

  $progressPath=Join-Path $dir "PROGRESS.md";if(!(Test-Path -LiteralPath $progressPath)){Write-Utf8 $progressPath "# Progress`r`n`r`n"}
  $findingsPath=Join-Path $dir "FINDINGS.md";if(!(Test-Path -LiteralPath $findingsPath)){Write-Utf8 $findingsPath "# Findings and decisions`r`n`r`n"}
  $git=Get-GitSnapshot $Root
  $active=$null;if($State.active_task_id){$active=Get-Task $State ([string]$State.active_task_id)}
  $next=Get-NextReadyTask $State
  $nextAction="Reconcile blockers/completion; do not invent a new task without evidence."
  if($State.safe_stop_requested){$nextAction="Do not start new work. Resume only after quota/session guard clears."}
  elseif($active -and $active.status -eq "doing"){$nextAction=("Continue {0} from its task card, then verify and checkpoint." -f $active.id)}
  elseif($next){$nextAction=("Start {0} - {1} after preflight." -f $next.id,$next.title)}
  $activeSummary="none";if($active){$activeSummary=("{0} - {1} [{2}]" -f $active.id,$active.title,$active.status)}
  ...
  $resume.Add(("- Mission ID: {0}" -f $State.mission_id));$resume.Add(("- Mission status: **{0}**" -f $State.status))
  $resume.Add(("- Gate: {0}" -f $State.current_gate));$resume.Add(("- Checkpoint reason: {0}" -f $CheckpointReason))
  $resume.Add(("- Checkpoint time: {0}" -f $State.last_checkpoint_at));$resume.Add(("- Active task: **{0}**" -f $activeSummary))
  $resume.Add(("- Safe stop requested: **{0}**" -f $State.safe_stop_requested));$resume.Add(("- Safe stop reason: {0}" -f $State.safe_stop_reason));$resume.Add("")
  $resume.Add("## Quota snapshot");$resume.Add("")
  $quotaSource="unavailable";$ordinary="";$reached=""
  if($q){if($q.source){$quotaSource=[string]$q.source};if($null -ne $q.ordinary_usage_allowed){$ordinary=[string]$q.ordinary_usage_allowed};if($q.rate_limit_reached_type){$reached=[string]$q.rate_limit_reached_type}}
  $resume.Add(("- Source: {0}" -f $quotaSource));$resume.Add(("- 5h remaining: **{0}**" -f $five));$resume.Add(("- Weekly remaining: **{0}**" -f $week))
  $resume.Add(("- Backend ordinary usage allowed: {0}" -f $ordinary));$resume.Add(("- Reached type: {0}" -f $reached));$resume.Add("")
  $resume.Add("## Exact next action");$resume.Add("");$resume.Add(("**{0}**" -f $nextAction));$resume.Add("")
  $resume.Add("## Recovery protocol");$resume.Add("")
  $resume.Add("1. Read this file first.");$resume.Add("2. Read MISSION.md and TASKS.md.");$resume.Add("3. Read only the active/next task card under tasks/.")
  $resume.Add("4. Read the tail of PROGRESS.md and FINDINGS.md; do not replay the whole history unless necessary.")
  $resume.Add("5. Reconcile the Git snapshot below with the live working tree.");$resume.Add("6. Run Mission resume, then Mission preflight before heavy work.")
  ...
  $resumeCommand='    & "'+$missionScript+'" resume -Root "'+$Root+'"';$resume.Add($resumeCommand);$resume.Add("")
  $resume.Add("## Git snapshot");$resume.Add("");$resume.Add(("- Branch: {0}" -f $git.branch));$resume.Add("")
  ...
  Add-MarkdownSection $resume "## Mission goal" ([string]$State.goal);Add-MarkdownSection $resume "## Success condition" ([string]$State.success)
  ...
  if($State.safe_stop_requested){
    $stopLines=@("# SAFE STOP REQUESTED","","Do not start new implementation, routed workers, broad verification, migration, installation, or destructive work until Mission resume/preflight clears this state.","",("Reason: {0}" -f $State.safe_stop_reason),("Checkpoint: {0}" -f $State.last_checkpoint_at),"Resume file: RESUME-FROM-HERE.md")
    Write-Utf8 $stopPath (($stopLines -join "`r`n")+"`r`n")
  }elseif(Test-Path -LiteralPath $stopPath){Remove-Item -LiteralPath $stopPath -Force -ErrorAction SilentlyContinue}
}
...
function Validate-MissionPlan($State){
  ...
  foreach($t in @($State.tasks)){
    ...
    $taskId=[string]$t.id
    if($ids.ContainsKey($taskId)){$errors.Add(("duplicate task id: {0}" -f $taskId))}else{$ids[$taskId]=$t}
    if([string]::IsNullOrWhiteSpace([string]$t.title)){$errors.Add(("{0}: missing task title" -f $taskId))}
    if([string]::IsNullOrWhiteSpace([string]$t.why)){$errors.Add(("{0}: missing why" -f $taskId))}
    if([string]::IsNullOrWhiteSpace([string]$t.verify) -and $t.status -notin @("deferred","out-of-scope")){$errors.Add(("{0}: missing verification" -f $taskId))}
    if([string]::IsNullOrWhiteSpace([string]$t.best_model) -and $t.status -notin @("deferred","out-of-scope")){$errors.Add(("{0}: missing model route" -f $taskId))}
  }
  foreach($t in @($State.tasks)){
    $taskId=[string]$t.id
    foreach($d in @($t.depends_on)){
      if(!$d){continue};$dep=[string]$d
      if($dep -eq $taskId){$errors.Add(("{0}: self dependency" -f $taskId))}elseif(!$ids.ContainsKey($dep)){$errors.Add(("{0}: missing dependency {1}" -f $taskId,$dep))}
    }
  }
  ...
  foreach($t in $active){$taskId=[string]$t.id;$ind[$taskId]=0;$edges[$taskId]=New-Object 'Collections.Generic.List[string]'}
  foreach($t in $active){$taskId=[string]$t.id;foreach($d in @($t.depends_on)){$dep=[string]$d;if($ind.ContainsKey($dep)){$ind[$taskId]=[int]$ind[$taskId]+1;$edges[$dep].Add($taskId)}}}
  $queue=New-Object 'Collections.Generic.Queue[string]';foreach($key in @($ind.Keys)){if([int]$ind[$key] -eq 0){$queue.Enqueue([string]$key)}}
  $processed=0
  while($queue.Count){$x=$queue.Dequeue();$processed++;foreach($n in $edges[$x]){$ind[$n]=[int]$ind[$n]-1;if([int]$ind[$n] -eq 0){$queue.Enqueue([string]$n)}}}
